...
class ModelPredictiveController(Controller):
    ...
    def _init_variables(self):
        ...
        self.ds = self.opti.parameter(
            self.N
        )  # ds trajectory var (just for loggin purposes)
    ...
    def _stage_constraints(self, n):
        ...
        self.opti.subject_to(
            self.state[:, n + 1]
            == self.car.spatial_transition(
                state, action, self.curvature[n], self.ds[n]
            )
        )
    ...
    def command(self, state):
        ...
        action = KinematicCarAction(
            a=self.action_prediction[0][0], w=self.action_prediction[1][0]
        )
        return action
    ...
    def _init_horizon(self, state):
        ...
        ds_traj = (
            np.full(self.N + 1, self.config.mpc_dt)
            * self.state_prediction[self.car.state.index("v"), :]
            + 0.5
        )
        self.opti.set_value(self.ds, ds_traj[:-1])
        ds_traj[0] = 0
        s_traj = np.cumsum(ds_traj) + state[self.car.state.index("s")]
        s_traj = s_traj[:-1]
        self.opti.set_value(self.curvature, self.car.track.k(s_traj))

    def get_state_prediction(self):
        preds_car = [
            self.car.rel2glob(self.state_prediction[:, i])
